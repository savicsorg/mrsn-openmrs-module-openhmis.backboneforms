    ...
    function (Backbone) {
        ...
        editors.List = editors.Base.extend({
            ...
            validate: function () {
                ...
                var fieldError = {
                    type: 'list',
                    message: openhmis.getMessage('Some of the items in the list failed validation'),
                    errors: errors
                };
        ...
        editors.List.Modal = editors.List.Object = editors.List.NestedModel = editors.Base.extend({
            ...
            initialize: function (options) {
                ...
                //Dependencies
                if (!editors.List.Modal.ModalAdapter) {
                    throw 'A ModalAdapter is required';
                }

                //Get nested schema if Object
                if (schema.itemType == 'Object') {
                    if (!schema.subSchema) {
                        throw 'Missing required option "schema.subSchema"';
                    }
                ...
                if (schema.itemType == 'NestedModel') {
                    ...
                    // Ensure that model type is loaded
                    var temp = new schema.model;

                    this.nestedSchema = schema.model.prototype.schema;
                    if (_.isFunction(this.nestedSchema)) {
                        this.nestedSchema = this.nestedSchema();
                    }
                }
            },
